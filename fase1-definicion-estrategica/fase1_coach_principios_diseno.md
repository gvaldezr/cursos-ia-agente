...
> **Cada módulo debe producir un output que el directivo pueda usar HOY en su trabajo real antes de que hayan pasado 10 minutos desde que empezó.**
...
| Módulo | Output usable en < 10 min |
|--------|---------------------------|
...
> **Si no puedes explicar el concepto durante un almuerzo con un colega no-técnico, no lo expliques así en el curso.**

### Vocabulario prohibido vs. permitido:

| ❌ NUNCA digas | ✅ SÍ puedes decir |
|---|---|
...
| "Fine-tuning" | "Entrenamiento especializado — como cuando capacitas a alguien para tu área específica" |
...
| "Retrieval-Augmented Generation" | "Cuando la IA busca en TUS documentos antes de responder, en vez de responder de memoria" |
...
1. **Analogía primero**: "Es como cuando le das una carpeta a tu asistente..."
2. **Nombre coloquial**: "...a eso se le llama 'IA con tus documentos'"
...
|---|---|
| "Paso 2 horas escribiendo el informe mensual para rectoría" | "Sube tu reporte de datos y pídele un resumen ejecutivo de 1 página" |
...
| "Me pidieron un benchmarking y no tengo tiempo de investigar" | "Pídele un análisis comparativo de tu programa vs. competidores en la región" |
| "Necesito un comunicado urgente y no tengo al diseñador" | "Genera el borrador en 2 minutos y solo ajusta el tono" |
| "La junta es en 2 horas y no tengo la presentación lista" | "Sube tus notas y pídele una estructura de presentación con puntos clave" |
...
Enseñar en cada módulo el hábito **PAUS** (Pregunta Antes de Usar la Salida):
...
**Cada módulo a partir del 4 incluye un "Momento PAUS":**
...
- Reflexión: "¿Lo habrías detectado si no te hubiéramos pedido que revisaras?"
...
```
FASE 1 (Módulos 1-3): "YO puedo usar esto"
...
FASE 2 (Módulos 4-6): "YO puedo confiar en esto (con cuidado)"
...
FASE 3 (Módulos 7-8): "MI EQUIPO puede usar esto"
...
> **Nadie lidera una transformación que no ha vivido primero.** Por eso los módulos de "estrategia" son los ÚLTIMOS, no los primeros.
...
Diseñar cada módulo asumiendo este participante:
...
- **Motivación real**: Si esto no le ahorra tiempo tangible, no regresa al módulo 2
...
| **Escape** | Siempre hay un "si esto no te funcionó, prueba esto otro" |
...
Antes de publicar cada módulo, preguntar:
...
## PRINCIPIO 7: Cada Módulo es Autocontenido pero Acumulativo

### Problema real:
No todos los directivos completarán los 8 módulos en orden. Algunos empezarán por el que les parece más útil. Otros abandonarán después del tercero. Otros harán solo el 1 y el 7.

### Solución de diseño:

**Autocontenido**: Cada módulo tiene sentido por sí mismo. Si alguien solo hace el Módulo 4, obtiene valor real.

**Acumulativo**: Cada módulo incluye un mini-recordatorio de lo que se aprendió antes:
- "En módulos anteriores aprendiste a dar instrucciones claras. Ahora vamos a..."
- "Si no hiciste el módulo anterior, aquí tienes lo mínimo que necesitas saber: [recuadro resumen de 3 líneas]"

**Enganche hacia adelante**: Cada módulo termina con un adelanto del siguiente:
- "Ahora sabes pedir bien. Pero, ¿qué pasa cuando la IA te responde con información falsa? Eso lo veremos en el siguiente módulo..."

### Mapa de dependencias:

```
Módulo 1 ──→ Módulo 2 ──→ Módulo 3     (secuencia recomendada)
                              │
                              ▼
              Módulo 4 ──→ Módulo 5     (secuencia fuerte)
                              │
                              ▼
                         Módulo 6       (integración)
                              │
                              ▼
              Módulo 7 ──→ Módulo 8     (secuencia fuerte)

Dependencias FUERTES (no saltar):
  • Módulo 5 requiere haber hecho 3 o 4
  • Módulo 7 requiere haber hecho al menos 3
  • Módulo 8 requiere haber hecho 6 o 7

Dependencias SUAVES (recomendadas pero no obligatorias):
  • Módulo 4 sin haber hecho 3 = posible pero menos profundo
  • Módulo 6 sin haber hecho 5 = posible pero sin pensamiento crítico
```
...
### Anti-patrón:
Tener un "Módulo de Ética" separado al final que nadie ve, o una diapositiva de "consideraciones éticas" que se salta.
...
Cada módulo incluye un punto donde el participante se enfrenta a una decisión ética en el ejercicio mismo:

| Módulo | "Momento Incómodo" |
|--------|---------------------|
| 1 | "La IA te acaba de generar un análisis brillante. ¿Lo presentas como tuyo o mencionas que lo hizo la IA?" |
| 2 | "Necesitas un comunicado urgente. ¿Copias y pegas lo que generó la IA sin leerlo completo?" |
| 3 | "Tu prompt incluye los nombres reales de 3 docentes con bajo desempeño. ¿Está bien poner esos nombres en una herramienta en la nube?" |
...
| 6 | "Tu equipo dice que la IA puede hacer el trabajo de la persona que contesta llamadas. ¿Es ético automatizar ese puesto?" |
| 7 | "Creaste un agente que responde a alumnos. Un alumno en crisis emocional habla con el agente. ¿Qué protocolo necesitas?" |
| 8 | "Presentas tu plan de IA al rector. Un colega pregunta: ¿y si esto elimina puestos de trabajo? ¿Qué respondes?" |
...
2. **Progreso visible**. Barra de avance del módulo Y del curso completo. El directivo debe saber en todo momento dónde está y cuánto le falta.
...
| 3 | Biblioteca de 3 prompts RCTE con outputs |
...
### Certificación:
- **Participante**: Completa módulos 1-5 con evidencias
- **Practitioner**: Completa módulos 1-7 con evidencias
- **Leader**: Completa los 8 módulos. Presenta su plan de adopción. Es capaz de capacitar a otros.
...
| 7 | Autocontenido + acumulativo | ¿Cada módulo da valor solo y también en secuencia? |
